...
function accessRolesV7_(){return ['EMPLOYEE','SUPERVISOR','MANAGER','ADMIN']}
function managementRolesV7_(){return ['SUPERVISOR','MANAGER','ADMIN']}
function normalizeAccessRoleV7_(role,fallback){const value=String(role||'').toUpperCase();return accessRolesV7_().indexOf(value)>=0?value:String(fallback||'EMPLOYEE').toUpperCase()}
function validateAccessRoleV7_(role){const value=String(role||'EMPLOYEE').toUpperCase();if(accessRolesV7_().indexOf(value)<0)throw new Error('Access Role ไม่ถูกต้อง');return value}
function isManagementRoleV7_(role){return managementRolesV7_().indexOf(String(role||'').toUpperCase())>=0}
function portalLoginV7_(payload) {
  ...
  return {ok:true,token:token,expiresIn:PORTAL_TOKEN_TTL_SECONDS_,employee:portalEmployeeSafeV7_(emp),scopes:isManagementRoleV7_(emp.accessRole)?managerScopesForEmployeeV7_(emp.id):[],serverEpochMs:Date.now()};
}
...
function portalBootstrapV7_(payload){const e=requirePortalV7_(payload.portalToken);return {employee:portalEmployeeSafeV7_(e),branches:getBranchesV7_(false),departments:getDepartmentsV7_(false),shifts:getShiftsV7_(false),scopes:isManagementRoleV7_(e.accessRole)?managerScopesForEmployeeV7_(e.id):[],notifications:unreadNotificationCountV7_(e.id),serverEpochMs:Date.now()}}
...
function requireManagerScopeV7_(portalToken, branchCode, departmentCode, permission) {
  const emp=requirePortalV7_(portalToken,managementRolesV7_());
  const scopes=managerScopesForEmployeeV7_(emp.id);
  const permissions=Array.isArray(permission)?permission:[permission],hit=scopes.find(s=>(s.branchCode==='*'||s.branchCode===String(branchCode||''))&&(s.departmentCode==='*'||s.departmentCode===String(departmentCode||''))&&(!permission||permissions.some(p=>p&&s[p]!==false)));
  if(!hit)throw new Error('ไม่มีสิทธิ์ดูแลสาขา/แผนกนี้');
  return {employee:emp,scope:hit};
}
function resolveManagerScopeSelectionV7_(scopes,branchCode,departmentCode,permission){
  const usable=(scopes||[]).filter(s=>s.active!==false&&(!permission||s[permission]!==false)),branchCodes=getBranchesV7_(false).map(x=>x.code),departmentCodes=getDepartmentsV7_(false).map(x=>x.code);
  let branch=String(branchCode||'');
  if(!branch||branch==='*'||!usable.some(s=>s.branchCode==='*'||s.branchCode===branch)){const fixed=usable.find(s=>s.branchCode&&s.branchCode!=='*');branch=fixed?fixed.branchCode:(usable.some(s=>s.branchCode==='*')?branchCodes[0]||'':'')}
  const branchScopes=usable.filter(s=>s.branchCode==='*'||s.branchCode===branch);
  let department=String(departmentCode||'');
  if(!department||department==='*'||!branchScopes.some(s=>s.departmentCode==='*'||s.departmentCode===department)){const fixed=branchScopes.find(s=>s.departmentCode&&s.departmentCode!=='*');department=fixed?fixed.departmentCode:(branchScopes.some(s=>s.departmentCode==='*')?departmentCodes[0]||'':'')}
  return{branchCode:branch,departmentCode:department};
}
...
function managerGetLeaveRequestsV7_(payload){const e=requirePortalV7_(payload.portalToken,managementRolesV7_()),scopes=managerScopesForEmployeeV7_(e.id).filter(s=>s.canApproveLeave!==false),all=leaveRowsV7_({status:String(payload.status||'PENDING')}).rows,rows=all.filter(r=>scopes.some(s=>(s.branchCode==='*'||s.branchCode===r.branchCode)&&(s.departmentCode==='*'||s.departmentCode===r.departmentCode)));return{rows:rows,serverEpochMs:Date.now()}}
...
function managerGetAttendanceCorrectionsV7_(payload){const e=requirePortalV7_(payload.portalToken,managementRolesV7_()),scopes=managerScopesForEmployeeV7_(e.id).filter(s=>s.canViewAttendance!==false),rows=correctionRowsV7_('',String(payload.status||'PENDING')).rows.filter(r=>scopes.some(s=>(s.branchCode==='*'||s.branchCode===r.branchCode)&&(s.departmentCode==='*'||s.departmentCode===r.departmentCode)));return{rows:rows}}
...
function managerGetShiftSwapRequestsV7_(payload){const e=requirePortalV7_(payload.portalToken,managementRolesV7_()),scopes=managerScopesForEmployeeV7_(e.id).filter(s=>s.canManageSchedule!==false),rows=swapRowsV7_('',String(payload.status||'PENDING_MANAGER')).filter(r=>scopes.some(s=>(s.branchCode==='*'||s.branchCode===r.branchCode)&&(s.departmentCode==='*'||s.departmentCode===r.departmentCode)));return{rows:rows}}
...
function adminSaveManagerScopeV7_(payload){
  requireAdmin_(String(payload.adminToken||''));
  const x=payload.scope||{},emp=employeeRecordV7_(String(x.employeeId||''));
  if(!emp)throw new Error('ไม่พบพนักงาน');
  const role=validateAccessRoleV7_(x.accessRole||emp.accessRole);
  if(!isManagementRoleV7_(role))throw new Error('Role นี้ไม่สามารถใช้ Manager Scope ได้');
  const branch=String(x.branchCode||'*'),dept=String(x.departmentCode||'*');
...
function portalHomeBootstrapV7_(payload){
  ...
  const bootstrap={employee:portalEmployeeSafeV7_(e),branches:getBranchesV7_(false),departments:getDepartmentsV7_(false),shifts:getShiftsV7_(false),scopes:isManagementRoleV7_(e.accessRole)?managerScopesForEmployeeV7_(e.id):[],notifications:unreadNotificationCountV7_(e.id),serverEpochMs:Date.now()};
...
function managerDashboardBundleV7_(payload){
  const summary=managerSummaryV7_(payload),scopes=summary.scopes||[],selected=resolveManagerScopeSelectionV7_(scopes,payload.branchCode,payload.departmentCode),branch=selected.branchCode,dept=selected.departmentCode;
  const data={summary:summary,branchCode:branch,departmentCode:dept,team:[],leave:[],corrections:[],swaps:[],daily:[]};if(branch&&dept){const canView=scopes.some(s=>(s.branchCode==='*'||s.branchCode===branch)&&(s.departmentCode==='*'||s.departmentCode===dept)&&s.canViewAttendance!==false);if(canView)data.team=managerGetTeamV7_({portalToken:payload.portalToken,branchCode:branch,departmentCode:dept}).rows||[];data.leave=managerGetLeaveRequestsV7_({portalToken:payload.portalToken,status:'PENDING'}).rows||[];data.corrections=managerGetAttendanceCorrectionsV7_({portalToken:payload.portalToken,status:'PENDING'}).rows||[];data.swaps=managerGetShiftSwapRequestsV7_({portalToken:payload.portalToken,status:'PENDING_MANAGER'}).rows||[];data.daily=managerGetDailyChecksV7_({portalToken:payload.portalToken}).rows||[]}return data;
}
function managerScheduleBundleV7_(payload){
  const e=requirePortalV7_(payload.portalToken,managementRolesV7_()),scopes=managerScopesForEmployeeV7_(e.id),selected=resolveManagerScopeSelectionV7_(scopes,payload.branchCode,payload.departmentCode,'canManageSchedule'),branch=selected.branchCode,dept=selected.departmentCode,week=weekStartKeyV7_(payload.weekStart||Utilities.formatDate(new Date(),TZ,'yyyy-MM-dd'));
