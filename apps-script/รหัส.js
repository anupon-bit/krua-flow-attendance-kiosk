...
function adminGetFastSummary_(token, forceFresh) {
  ...
  if (empSh && empSh.getLastRow() >= 2) {
    employeeRows = empSh.getRange(2,1,empSh.getLastRow()-1,31).getValues();
  }
...
function portalLoginV7_(payload) {
  ...
  return {ok:true,token:token,expiresIn:PORTAL_TOKEN_TTL_SECONDS_,employee:portalEmployeeSafeV7_(emp),scopes:emp.accessRole==='MANAGER'?managerScopesForEmployeeV7_(emp.id):[],serverEpochMs:Date.now()};
}
...
function portalBootstrapV7_(payload){const e=requirePortalV7_(payload.portalToken);return {employee:portalEmployeeSafeV7_(e),branches:getBranchesV7_(false),departments:getDepartmentsV7_(false),shifts:getShiftsV7_(false),scopes:e.accessRole==='MANAGER'?managerScopesForEmployeeV7_(e.id):[],notifications:unreadNotificationCountV7_(e.id),serverEpochMs:Date.now()}}
...
function managerScopesForEmployeeV7_(employeeId) {
  ...
  return cacheJsonV7_('V7_SCOPES_'+id,60,()=>{
    ...
    return sh.getRange(2,1,last-1,11).getValues().filter(r=>String(r[1])===id&&r[7]!==false).map(r=>({scopeId:String(r[0]),employeeId:String(r[1]),branchCode:String(r[2]),departmentCode:String(r[3]),canApproveLeave:r[4]!==false,canManageSchedule:r[5]!==false,canViewAttendance:r[6]!==false,active:r[7]!==false,note:String(r[10]||'')}));
  });
}
function requireManagerScopeV7_(portalToken, branchCode, departmentCode, permission) {
  const emp=requirePortalV7_(portalToken,['MANAGER']);
...
function managerGetLeaveRequestsV7_(payload){const e=requirePortalV7_(payload.portalToken,['MANAGER']),scopes=managerScopesForEmployeeV7_(e.id).filter(s=>s.canApproveLeave!==false),all=leaveRowsV7_({status:String(payload.status||'PENDING')}).rows,rows=all.filter(r=>scopes.some(s=>(s.branchCode==='*'||s.branchCode===r.branchCode)&&(s.departmentCode==='*'||s.departmentCode===r.departmentCode)));return{rows:rows,serverEpochMs:Date.now()}}
...
function managerGetAttendanceCorrectionsV7_(payload){const e=requirePortalV7_(payload.portalToken,['MANAGER']),scopes=managerScopesForEmployeeV7_(e.id).filter(s=>s.canViewAttendance!==false),rows=correctionRowsV7_('',String(payload.status||'PENDING')).rows.filter(r=>scopes.some(s=>(s.branchCode==='*'||s.branchCode===r.branchCode)&&(s.departmentCode==='*'||s.departmentCode===r.departmentCode)));return{rows:rows}}
...
function managerGetShiftSwapRequestsV7_(payload){const e=requirePortalV7_(payload.portalToken,['MANAGER']),scopes=managerScopesForEmployeeV7_(e.id).filter(s=>s.canManageSchedule!==false),rows=swapRowsV7_('',String(payload.status||'PENDING_MANAGER')).filter(r=>scopes.some(s=>(s.branchCode==='*'||s.branchCode===r.branchCode)&&(s.departmentCode==='*'||s.departmentCode===r.departmentCode)));return{rows:rows}}
...
function adminSetEmployeeAccessRoleV7_(payload){requireAdmin_(String(payload.adminToken||''));const e=employeeRecordV7_(String(payload.employeeId||'')),role=String(payload.accessRole||'EMPLOYEE').toUpperCase();if(!e)throw new Error('ไม่พบพนักงาน');if(['EMPLOYEE','MANAGER'].indexOf(role)<0)throw new Error('Role ไม่ถูกต้อง');const sh=SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(EMPLOYEE_SHEET);sh.getRange(e.row,33).setValue(role);sh.getRange(e.row,6).setValue(new Date());auditLogV7_('ADMIN','ADMIN','SET_ACCESS_ROLE','EMPLOYEE',e.id,{accessRole:e.accessRole},{accessRole:role},String(payload.note||''),String(payload.requestId||''));return{ok:true}}
function adminGetManagerScopesV7_(payload){requireAdmin_(String(payload.adminToken||''));const sh=SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(MANAGER_SCOPES_SHEET),last=sh?sh.getLastRow():0;if(!sh||last<2)return{rows:[]};const rows=sh.getRange(2,1,last-1,11).getValues().filter(r=>!payload.employeeId||String(r[1])===String(payload.employeeId)).map(r=>({scopeId:String(r[0]),employeeId:String(r[1]),branchCode:String(r[2]),departmentCode:String(r[3]),canApproveLeave:r[4]!==false,canManageSchedule:r[5]!==false,canViewAttendance:r[6]!==false,active:r[7]!==false,note:String(r[10]||'')}));return{rows:rows}}
function adminSaveManagerScopeV7_(payload){requireAdmin_(String(payload.adminToken||''));const x=payload.scope||{},emp=employeeRecordV7_(String(x.employeeId||''));if(!emp)throw new Error('ไม่พบพนักงาน');const branch=String(x.branchCode||'*');if(branch!=='*')validateBranchV7_(branch,false);const dept=String(x.departmentCode||'*');if(dept!=='*')validateDepartmentV7_(dept,false);const id=String(x.scopeId||'')||'SCP-'+Utilities.getUuid().slice(0,10).toUpperCase(),sh=SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(MANAGER_SCOPES_SHEET),row=findRowByIdV7_(sh,id),now=new Date(),vals=[id,emp.id,branch,dept,x.canApproveLeave!==false,x.canManageSchedule!==false,x.canViewAttendance!==false,x.active!==false,row?sh.getRange(row,9).getValue()||now:now,now,String(x.note||'')];if(row)sh.getRange(row,1,1,11).setValues([vals]);else sh.appendRow(vals);const esh=SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName(EMPLOYEE_SHEET);esh.getRange(emp.row,33).setValue('MANAGER');cacheRemoveV7_(['V7_SCOPES_'+emp.id]);auditLogV7_('ADMIN','ADMIN','SAVE_MANAGER_SCOPE','MANAGER_SCOPE',id,'',x,String(x.note||''),String(payload.requestId||''));return{ok:true,scopeId:id}}
...
function managerScheduleBundleV7_(payload){
  const e=requirePortalV7_(payload.portalToken,['MANAGER']),scopes=managerScopesForEmployeeV7_(e.id);let branch=String(payload.branchCode||''),dept=String(payload.departmentCode||'');if(!branch){const s=scopes.find(x=>x.branchCode!=='*')||scopes[0];branch=s?s.branchCode:''}if(!dept){const s=scopes.find(x=>(x.branchCode==='*'||x.branchCode===branch)&&x.departmentCode!=='*')||scopes.find(x=>x.departmentCode!=='*')||scopes[0];dept=s?s.departmentCode:''}const week=weekStartKeyV7_(payload.weekStart||Utilities.formatDate(new Date(),TZ,'yyyy-MM-dd'));
